...
def _streak_score(text) -> tuple[int, int]:
    """解析火花文本，返回 (可靠性, 天数)。

    - 纯数字（如 "63" / "🔥 63"）：可靠性 1，天数=数字；
    - 倒计时格式（如 "2 天后消失" / "6 天后消失"）：可靠性 0，天数=数字。
    同一联系人重复出现时优先采用可靠性高的真实天数，避免倒计时覆盖真实值。
    """
    t = str(text or "").strip()
    m = re.search(r"\d+", t)
    if not m:
        return 0, 0
    val = int(m.group())
    if "消失" in t or "天后" in t:
        return 0, val
    return 1, val


def _dedup_contacts(contacts: list[dict]) -> list[dict]:
    """同一显示名在滚动采集时可能重复出现（真实天数 + 倒计时两种文案），
    聚合为一条：优先真实天数，同类取最大，避免低值/倒计时覆盖真实火花天数。"""
    agg: dict[str, dict] = {}
    for c in contacts or []:
        name = str(c.get("name", "")).strip()
        if not name:
            continue
        rel, val = _streak_score(c.get("streak"))
        prev = agg.get(name)
        if prev is None:
            agg[name] = {"name": name, "streak": val, "reliability": rel, "avatar": c.get("avatar") or ""}
        else:
            if rel > prev["reliability"]:
                prev["streak"], prev["reliability"] = val, rel
            elif rel == prev["reliability"] and val > prev["streak"]:
                prev["streak"] = val
            if not prev.get("avatar") and c.get("avatar"):
