"""Better Douyin Streak：多账号抖音续火花 Web 服务入口。
...
def _dashboard_data(account_id: str) -> dict:
    """聚合概览看板数据：汇总指标、14 天发送趋势、火花分布、Top 火花、近期运行。"""
    rt = load_runtime(account_id)
    entries = ledger.load_ledger(account_id)
    history = rt.get("history") or []

    now = datetime.now().astimezone()
    today = now.date()

    spark = [e for e in entries if (e.get("streak_days") or 0) > 0]
    ...
    for i in range(13, -1, -1):
        ...
        days.append({
            ...
            "ok": 0, "failed": 0, "skipped": 0, "runs": 0,
        })
    day_index = {row["date"]: row for row in days}

    ok_total = failed_total = skip_total = 0
    for run in history:
        at = _parse_at(run.get("at"))
        if at is None:
            continue
        ok_n = len(run.get("ok") or [])
        failed_n = len(run.get("failed") or [])
        skip_n = len(run.get("skipped") or [])
        ok_total += ok_n
        failed_total += failed_n
        skip_total += skip_n
        row = day_index.get(at.date().isoformat())
        if row:
            row["ok"] += ok_n
            row["failed"] += failed_n
            row["skipped"] += skip_n
    ...
    # 火花天数分布
    buckets = [
        {"key": "0", "label": "未点亮", "min": 0, "max": 0, "count": 0},
        {"key": "1-7", "label": "1~7 天", "min": 1, "max": 7, "count": 0},
        {"key": "8-30", "label": "8~30 天", "min": 8, "max": 30, "count": 0},
        {"key": "31-99", "label": "31~99 天", "min": 31, "max": 99, "count": 0},
        {"key": "100", "label": "100 天+", "min": 100, "max": 10**9, "count": 0},
    ]
    for e in entries:
        s = int(e.get("streak_days") or 0)
        for b in buckets:
            if b["min"] <= s <= b["max"]:
                b["count"] += 1
                break
    ...
    # Top 火花（按天数降序）
    top = sorted(entries, key=lambda e: -(e.get("streak_days") or 0))[:8]
    top_streaks = [
        {
            "display_name": e.get("display_name"),
            "nickname": e.get("nickname") or "",
            "streak_days": int(e.get("streak_days") or 0),
    ...
    recent_runs = [
        {
            ...
            "logged_out": bool(r.get("logged_out")),
            "rate_limited": bool(r.get("rate_limited")),
        }
        for r in history[:10]
    ]

    max_streak = max((e.get("streak_days") or 0) for e in entries) if entries else 0
    avg_streak = round(
        sum(int(e.get("streak_days") or 0) for e in spark) / len(spark), 1
    ) if spark else 0
    week_ago = today - timedelta(days=7)
    success_runs_7d = sum(
        1 for r in history
        if r.get("ok") and not r.get("failed")
        and (at := _parse_at(r.get("at"))) and at.date() >= week_ago
    )
    ...
    summary = {
        ...
        "no_conversation": sum(1 for e in entries if not e.get("has_conversation")),
        "max_streak": max_streak,
        "avg_streak": avg_streak,
        "ok_sends_total": ok_total,
        "failed_sends_total": failed_total,
        ...
        "harvest_last": load_harvest_last(account_id),
    }
...
app = FastAPI(title="Better Douyin Streak", lifespan=lifespan)
...
def api_dashboard(
    ...
    """概览看板聚合数据：汇总指标、发送趋势、火花分布、Top 火花、近期运行。"""
    _check_auth(token)
    aid = _resolve_account(account_id)
    rt = load_runtime(aid)
    data = _dashboard_data(aid)
    data.update({
        "account_id": aid,
